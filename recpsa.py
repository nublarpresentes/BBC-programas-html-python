...
def cadastrar_recpsa():
    ...
    try:
        ...
        cur.execute("""
            ...
            ("idTipRecpa","idAssent","valRecpsa","qtdEqv","idPolPub","idTipUsoInfr")
            VALUES (%s,%s,%s,%s,%s,%s)
        """, (int(idTipRecpa), int(idAssent), valRecpsa, qtdEqv,
        ...
        try:
            ...
            pass
        print("Erro ao cadastrar recompensa:", e)
    ...
    try:
        ...
        if int(idTipRecpa) == 1:
            ...
            try:
                ...
                valRecpsa = 0.0
            idTipUsoInfr = None
            upd = ('UPDATE "tbrecpsa" SET "idTipRecpa"=%s,"idAssent"=%s,"valRecpsa"=%s,"qtdEqv"=%s,"idPolPub"=%s,"idTipUsoInfr"=NULL WHERE "idRecpsa"=%s',
                   (int(idTipRecpa), int(idAssent), valRecpsa, qtdEqv, int(idPolPub), int(idRecpsa)))
        else:
            if not idTipUsoInfr:
                conn.close()
                flash('❌ Escolha o tipo de uso de infraestrutura.', 'danger')
            ...
            valRecpsa = float(inf[2] or 0)
            idPolPub = None
            upd = ('UPDATE "tbrecpsa" SET "idTipRecpa"=%s,"idAssent"=%s,"valRecpsa"=%s,"qtdEqv"=%s,"idPolPub"=NULL,"idTipUsoInfr"=%s WHERE "idRecpsa"=%s',
                   (int(idTipRecpa), int(idAssent), valRecpsa, qtdEqv, int(idTipUsoInfr), int(idRecpsa)))
